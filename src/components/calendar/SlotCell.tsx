...
export default function SlotCell({
    ...
    return (
        <div className={`border border-gray-200 rounded-md p-2 h-24 flex flex-col justify-between ${mine ? "bg-red-100" : members.length === 0 ? "bg-yellow-50" : "bg-blue-50"}`}>
            <div className="space-y-1 overflow-auto">
                {members.length === 0 ? (
                    <div className="text-xs text-gray-400 italic">
                        {disabled ? "This slot has either passed or is not within 2 weeks" : "`No one yet"}
                    </div>
                ) : (
                    members.map((m) => (
                        <div
